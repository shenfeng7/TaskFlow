...
任务数据原始文件是持久化的json，里面存储这任务的流程和每个任务的信息，基础任务数据通过注解运行时获取任务信息，前端请求基础任务信息，画出任务执行图，后端解析成为一个json 持久化到文件，可以做到轻量实时修改，也可以复用同时可以加密混淆文件方式直接通过直接改动文件以保证程序正常运行。
程序实际启动过程中会初始化map，直接保证O（1）的效率，也可以构造出不同的数据结构类型方便数据访问，并且都统一管理以保证线程安全，任务执行的记录可以持久化到数据库中方便反推定位bug。
当前初始化的map所需要的功能有
* 任务基础信息，key：taskId，value：任务json 信息
* 快速知道下一个任务，一个链表；
* 任务执行情况，暂缓数据结构
* 任务执行条件，如产品类型，时间点
* 感觉还有但是不知道写啥
...
### 3.3. 任务通知器
![notify](images/notify_2026-05-03.png)  
