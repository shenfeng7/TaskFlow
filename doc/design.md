...
3. 判断任务是否可以执行，这个就不做rpc，任务模块单独作为一个项目引入到其他项目里面去， 然后判断

### 3.3.2. 任务数据模块

收集任务处理状态，如当前任务处理到哪，内存存储（主要是不依赖数据库，太慢）

我要知道下一个任务是那个，是一个图，通知是以时间点到的， 这个时间点要执行很多任务，里面得有应该执行时间之内按照时间来，一溜全部执行下去，第二个就是点名任务执行但是不一定能够执行，得判断
所以更像是个链表，可以考虑下，实例化之后变成一个链表，但是我可能有多个头，研究下。。。
...
* 状态机?
* 邻接表 + hash 索引
设计两个数据结构并存，初始化会很慢就是了
事件通知之后需要判断那个任务要在这个时间点执行，没这个我没办法知道要执行那个任务， 所以这个数据结构核心就是帮我判断那个任务需要执行，设计还是一条线，不用所有任务都判定，因为这个执行顺序已经被整理好，对应事件过来拿第一个判定就行，第二个就是调整不会太频繁，我如何存储这个流程到内存中，目前是一个方便存储，第二个就是初始化方便读取了。
存储：图，边，和点统计共性的，这个放弃，我没记错的，这个顺序实际上不是很重要，只是说依赖前一个任务的数据。对于这个来说按照时间点的方式来作为入口可以考虑一下，按照业务理解来说，就是一个定时任务差不多的东西，看着顺序也挺重要的。
读取：
暂时等会设计吧
...
任务是否需要抢夺， 还是直接判断， 实际上manager 并不知道executor 的实际情况， 我也没有设计什么任务队列让其进行抢夺， 所以当前就只是看那个executor 可以执行而已， 如何排这个优先级呢， 资源空闲程度，或者我需要两个executor 来帮我执行，任务拆分和分发，拆可以在写类的时候直接写出最小的粒度按照这个来发任务，但是我同时会有带有不同入参的任务都需要执行，所以肯定是会多个executor 一起执行任务的，每次任务分上之后需要重新计算优先级吗。需要重新计算优先级， 每次给最好的。增加手动模式（扩展）
...
任务处理器的状态管理，对接rpc， 管理线程池， 执行不同的任务步骤。
线程池的设计， 单线程接受任务，多线程执行，，需要保证有序么，需要保证有序，所以只能是单线程读取任务，分给多线程执行
...
单独写为一个模块，可以按照步骤级别的粒度来构成任务，每个步骤都获得一个可执行类和约束执行的时候需要去检查上一个任务步骤，如何去告知manager模块如何去分这些任务，也可以直接写来任务里面，如何去分，分成不同的集合然后执行，
...
把上面的语言整理一下，然后写代码设计， 同时把模块代码探索一下技术可行性
